...
public class ColaConsulta {
     ...
     * @param paciente Elemento a añadir
     */
    public void push(Paciente paciente) {
        if (isFull()) {
            expand();
        }
        pacientes[size] = paciente;
        size++;
    }
     ...
     */
    private void expand() {
        Paciente[] aux = new Paciente[Math.round(pacientes.length * GROW_FACTOR)];
     ...
     */
    public Paciente pop() {
        Paciente e = new Paciente(ERROR,ERROR, LocalDateTime.now());
        if (!isEmpty()) {
            e = pacientes[0];
            size--;
            moveToLeft();
